...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //根据身份信息获取角色信息，权限信息
        List<Role> roles=sysUserService.findUserByRoless(sysUser.getUsername());
        // 根据身份信息获取角色信息，权限信息
        for (Role role : roles) {
            simpleAuthorizationInfo.addRole(role.getName());
            System.out.println("roleName:"+role.getName());
            //根据身份信息获取权限信息
            List<Menu> menus=menuService.findRoleMenus(role.getName());
            for (Menu m : menus) {
                if(m.getPerms()!=null) {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
        ...
        try {
            ...
            Users sysUser = sysUserService.findUserByUserName(username);
            if (sysUser == null) {
                return null;
            }
